...
const UserHeader = ({userInfo}) => {  
  ...
  const handleDropdown = (label, url) => {
    if(label === "Logout") localStorage.removeItem("access_token_user"), dispatch(setUserInfo(null))
    navigate(url)
  }
  ...
  const items = (userLibraryHeader.infoDropdown).map(item => {
    return {
      key: item.url,
      label:<div onClick={ () => {handleDropdown(item.label, item.url)}} className="p-1" ><span>{item.label}</span></div>
    }
  })
  ...
  const itemsNav = (userLibraryHeader.nav).map(item => {
    return {
      key: item.url,
      label: <NavLink to={item.url} className={({ isActive }) => isActive ? "md:bg-[#F9C06A] md:block" : "" } ><span className="px-3 text-black">{item.label}</span></NavLink>
    }
  })
  ...
  return(
    <div className="header sticky top-0 z-50 bg-white md:shadow-none" id="header">
      <div className="header_main">
        <Row className="pageWrapper justify-between md:justify-start items-center" id="header_main">
          <Col span={6}><img src={img_logo} alt="" width={96} /></Col>
                ...
                <Input 
                  ...
                  suffix={
                    <div className="absolute w-6 top-2/4 -translate-y-2/4 right-0 -translate-x-2/4">
                      <img src={img_iconSearch} className=" opacity-50 cursor-pointer"/>
                    </div>
                  }
                />
              </Form.Item>
          ...
          <Col span={6} className="flex justify-end">
            <Row className="gap-3 hidden md:flex w-max ml-auto md:flex-nowrap">
              ...
              && 
                <>
                  <Link to={ROUTES.LOGIN} className="hover:text-black">{BtnYellow(userLibraryHeader.btn_login)}</Link>
                  <Link to={ROUTES.REGISTER} className="hover:text-black">{BtnYellow(userLibraryHeader.btn_register)}</Link>
                </>
              }
              {userInfo === null 
              && 
                <Skeleton className="flex items-center" avatar paragraph={{rows:0}} active style={{width:"200px"}} />
              }
              {userInfo
              &&
                <Dropdown menu={{items}} placement="bottomLeft">
                  <div className="flex items-center gap-3 cursor-pointer">
                    <Avatar src={checkAvatarUser(userInfo?.avatar)} size="large" />
            ...
            <Row justify="end" className="block md:hidden">
              <div className="flex justify-end cursor-pointer" onClick={showDrawer}><img src={img_navBar} /></div>
              <Drawer 
                title={
                  <div className="inline-block ml-5">
                    ...
                    { userInfo === null 
                    && 
                    <Skeleton className="w-2/4 flex items-center" avatar paragraph={{rows:1}} active width={200} />
                    }
                    { userInfo
                    &&
                    <Dropdown menu={{items}} placement="bottomLeft">
              ...
              >  
                <Menu mode="vertical" items={itemsNav} className="navHeader_mobile" />
              </Drawer>
            </Row>
          </Col>
        </Row>
        ...
        <div className="bg-[#f6f6f6] hidden md:block" id="userInfoHeader">
          <div className="pageWrapper" style={{paddingLeft:0, paddingRight:0}}>
            <Menu mode="horizontal" items={itemsNav} className="bg-[#f6f6f6] border-none" />
          </div>
        </div>
